...
function MedicalRecordCard({
  ...
  const renderQRSection = () => {
    ...
    // State C: REVIEW_REQUIRED (and not locally confirmed)
    if (hasUnverifiedMedicines && !isConfirmed) {
      return (
        <div className="flex items-center gap-2">
          <span className="font-bold text-amber-700 flex items-center gap-1 bg-amber-50 px-2 py-1 rounded border border-amber-100 text-[10px] sm:text-xs">
            ⚠ {unverifiedCount} medicines require review
          </span>
          <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowReviewModal(true); }} className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded transition-colors text-[10px] sm:text-xs">
            Review Prescription
          </button>
        </div>
      );
